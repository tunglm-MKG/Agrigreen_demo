...
| `data/backups/<thời điểm>/` | Đợt sao lưu: 7 tệp + thư mục `uploads/` + `manifest.json` (ghi cả định danh khoá mã hoá) | Tự chạy 5 phút sau khởi động và mỗi 24 giờ |
...
Biến môi trường: `BACKUP_DIR`, `BACKUP_KEEP` (mặc định 14), `BACKUP_INTERVAL_HOURS` (0 = tắt), `DATA_ENCRYPTION_KEY` (64 hex, thay cho tệp khoá), `UPLOAD_DIR` (thư mục ảnh; `attachments.storage_path` là khoá tương đối với thư mục này), `SQLITE_JOURNAL_MODE` (mặc định TRUNCATE), `SUPER_ADMIN_PASSWORD` (mật khẩu SAdmin ban đầu; không đặt → sinh tạm, in log một lần, bắt đổi), `DEMO_ACCOUNT_PASSWORD` / `SEED_DEMO_DATA` (tài khoản trình diễn — không nạp ở production), `DATA_ENCRYPTION_KEY_ID` + `DATA_ENCRYPTION_KEYS_PREVIOUS` (xoay khoá: `npm run rotate-key`), `TRUSTED_PROXY_HOPS` (số lớp proxy để đọc IP từ X-Forwarded-For; Render/Fly = 1), `WEBHOOK_ALLOWED_HOSTS` (allowlist máy chủ webhook SMS/email), `LOGIN_MAX_PER_WINDOW` (mặc định 20/10 phút), `LEGACY_HASH_DEADLINE` (mặc định 2026-12-31), `SCRYPT_N`, `SESSION_IDLE_MINUTES` (60), `MFA_ENFORCE` (production bật; bắt buộc TOTP cho quản trị), `MFA_ISSUER`, `REAUTH_MINUTES` (15), `AUDIT_STREAM` (`stdout` để đẩy nhật ký JSON Lines cho SOC), `BACKUP_INCLUDE_UPLOADS` (1), `SADMIN_RESET_EMAIL` (mặc định tunglm@mekonggreen.vn; rỗng/0 = tắt) + `RESET_LINK_MINUTES` (30) + `APP_BASE_URL` (địa chỉ công khai để dựng liên kết đặt lại), `SMTP_HOST`/`SMTP_PORT` (587; 465 = TLS ngầm)/`SMTP_SECURE`/`SMTP_STARTTLS`/`SMTP_USER`/`SMTP_PASS`/`SMTP_FROM`/`SMTP_TIMEOUT_MS`/`SMTP_TLS_REJECT_UNAUTHORIZED` (gửi email hệ thống; không có thì dùng webhook email của kênh thông báo, không có nữa thì ghi `data/outbox/*.eml`), `MAIL_OUTBOX_DIR`. Chi tiết: [CAP-DO-3-2026-09-24.md](CAP-DO-3-2026-09-24.md).
