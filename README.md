...
- **Email đặt lại mật khẩu SAdmin**: mỗi lần máy chủ khởi động lại và mỗi lần chạy `npm run reset-sadmin`, hệ thống gửi một email tới `SADMIN_RESET_EMAIL` (mặc định `tunglm@mekonggreen.vn`) chứa **liên kết đặt lại dùng một lần**, hiệu lực `RESET_LINK_MINUTES` (30) phút; thư không chứa mật khẩu, mật khẩu hiện tại vẫn dùng được cho tới khi liên kết được dùng. Gửi qua SMTP (`SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS`, `SMTP_FROM`) hoặc webhook email; chưa cấu hình thì thư được ghi vào `data/outbox/*.eml` và log cảnh báo. Liên kết trỏ về `APP_BASE_URL` (đặt đúng địa chỉ công khai khi triển khai).
